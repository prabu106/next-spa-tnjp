...
const footerLinks = ["Quick Links", "About Us", "Projects", "Contact Us"];
...
function Footer() {
  return (
    <footer className="bg-[linear-gradient(180deg,#153f7b_0%,#0c2f64_100%)] text-white">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-5 text-sm md:flex-row md:items-center md:justify-between lg:px-10">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-white/90">
          {footerLinks.map((link, index) => (
            <span key={link} className="flex items-center gap-5">
              <a href="#" className="transition hover:text-[#ffd18a]">
                {link}
              </a>
              {index < footerLinks.length - 1 ? <span className="text-white/30">|</span> : null}
            </span>
          ))}
        </div>
        <p className="text-white/85">
          © 2022 Student Project Centre. Privacy Policy · Terms of Use
        </p>
      </div>
    </footer>
  );
